...
class LevelsPopQuiz extends StatefulWidget {
  LevelsPopQuiz({
    Key? key,
  }) : super(key: key);

  @override
  _LevelsPopQuizState createState() => _LevelsPopQuizState();
}

class _LevelsPopQuizState extends State<LevelsPopQuiz>
                                                        ...
                                                        child: Center(
                                                          child: Column(
                                                            children: [
                                                              Text(
                                                                  'Explanation',
                                                                  style: AllTextStyles.workSansMedium().copyWith(
                                                                      fontWeight:
                                                                          FontWeight
                                                                              .bold,
                                                                      color: Colors
                                    ...
                                    Container(
                                        ...
                                        child: _animation.value >= 0.5
                                            ...
                                            : ElevatedButton(
                                                onPressed: () async {
                                                  userId =
                                                      Prefs.getString(PrefString.userId);
                                                  if (index ==
                                                      snapshot.data!.docs
                                                              .length -
                                                          1) {
  ...
  Future _whenLevelComplete({String? popQuiz, var userId, int? index}) async {
    ...
    if (popQuiz == 'Level_2_Pop_Quiz') {
      List referralUsersCount = documentSnapshot.get('referral_by');
      bool enterUserByPay = documentSnapshot.get('enter_via_paying');
      bool enterUserByInviteCode =
          documentSnapshot.get('enter_via_special_code');

      firestore.collection('User').doc(userId).update({
        'previous_session_info': (referralUsersCount.length == 3 ||
                enterUserByPay == true ||
        ...
        // if (value != true)
        //   'last_level': (referralUsersCount.length == 3 ||
        //           enterUserByPay == true ||
        //           enterUserByInviteCode == true)
        //       ? 'Level_3_setUp_page'
        //       : 'Level_3_setUp_page',
