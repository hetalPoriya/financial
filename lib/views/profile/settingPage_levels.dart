...
import 'package:financial/views/setupPage/level_five_setUp_page.dart';
import 'package:financial/views/setupPage/level_four_setUp_page.dart';
import 'package:financial/views/popQuiz/level_one_pop_quiz.dart';

import 'package:financial/views/setupPage/level_one_setUp_page.dart';
import 'package:financial/views/setupPage/level_three_setUp_page.dart';
import 'package:financial/views/setupPage/level_two_setUp_page.dart';
import 'package:financial/views/popQuiz/levels_pop_quiz.dart';
...
class SettingPageLevels extends StatefulWidget {
  const SettingPageLevels({Key? key}) : super(key: key);

  @override
  _SettingPageLevelsState createState() => _SettingPageLevelsState();
}

class _SettingPageLevelsState extends State<SettingPageLevels> {
  //for model
  QueModel? queModel;
  var userId;
  int gameScore = 0;
        ...
        : SafeArea(
            child: Scaffold(
              ...
              appBar: AppBar(
                ...
                elevation: 0,
                title: Text('Levels', style: AllTextStyles.settingsAppTitleInter()),
                centerTitle: true,
              ),
              body: Container(
                decoration: boxDecoration,
                                    ...
                                    child: Container(
                                      ...
                                      child: levelList[index].id! <= lev
                                          ? Align(
                                              ...
                                              child: Text(
                                                '${levelList[index].id}',
                                                style: AllTextStyles.balooDaSmall(),
                                              ),
                                            )
                                          : Container(
                                              height: 4.h,
  ...
  _updateValue(int? i) async {
    ...
    firestore.collection('User').doc(userId).update({
      ...
      if (i == 4) {
        ...
        await Prefs.setInt(
            PrefString.randomNumberValue, 0);
        await Prefs.setInt(
            PrefString.count, 0);
        SharedPreferences pref = await SharedPreferences.getInstance();
